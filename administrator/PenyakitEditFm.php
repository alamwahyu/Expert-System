...
if ($kdubah !=""){
	# Menampilkan Data
	$sql = "SELECT * FROM hamapenyakit WHERE Id_Pnykt='$kdubah'";
	$qry = mysql_query($sql, $koneksi)
		or die("SQL Error" .mysql_error());
	$data=mysql_fetch_array($qry);
	
	#Samakan dengan variabel form
	$TxtKodeH = $data['Id_Pnykt'];
	$TxtPenyakit=$data['JnsPnykt'];
...
<form action="PenyakitEditSim.php" method="post" name="form1" target="_self">
<table width="100%" border="0" align="center" cellpadding="2" cellspacing="1" bgcolor="">
<tr bgcolor=""> 
  <td>Kode</td>
  <td><input name="TxtKode" type="text"  maxlength="10" size="15" value="<? echo $TxtKodeH; ?>" disabled="disabled">
...
<tr bgcolor="">
  <td>Keterangan</td>
  <td><textarea name="TxtKeterangan" cols="40" rows="4" id="TxtKeterangan"><?= $TxtKeterangan; ?></textarea></td>
</tr>
<tr bgcolor=""> 
  <td>&nbsp;</td>
  <td><input type="submit" name="Submit" value="Simpan" class="btn btn-info"> <a href="javascript:history.back()" class="btn btn-warning">Batal</a>
  <label></label></td>
</tr>
</table>
</form>
