...
$tampil="select * from pages limit $posisi,$batas"; 
$hasil=mysql_query($tampil); 

echo "<table class='table table-striped table-condensed'>";
$no=$posisi+1; // Agar angka (penomoran) mengikuti paging 
while ($data=mysql_fetch_array($hasil)){ 
  echo "<tr><td>$no</td><td>".$data['tipe']."</td>
  <td>
    <a href='EditPages.php?id_pages=".$data['id_pages']."' class='btn'>Edit</a>
    <a href='HapusPages.php?id_pages=".$data['id_pages']."' class='btn'>Hapus</a>
  </td>
  </tr>"; 
  $no++; 
} 
...
echo "<p>Total Data Pages : <b>$jmldata</b> pages</p>"; 
?>
